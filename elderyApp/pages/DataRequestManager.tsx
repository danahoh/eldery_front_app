...
import {setReloadCookie, hasReloadCookie} from './CookieManager';
const API_KEY = 'AIzaSyDggsiCPhhVxF1ChkvW2dhRythu9p4jKRA';
const dataValues = [
  {
    title: 'Calories',
...
export const getWeeklyData = async (
  ...
  let state = [];
  let promises = [];
  const cookieInfo = await hasReloadCookie();
  const hasCookie = cookieInfo.present;

  if (!hasCookie || initialState.length === 0) {
    for (var i = 6; i >= 0; i--) {
